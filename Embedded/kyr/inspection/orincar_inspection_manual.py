...
ai_model_path = script_dir.parent.parent.parent / 'AI' / 'kyr' / 'src' / 'anomaly_detection' / 'models'
if ai_model_path.exists():
    sys.path.insert(0, str(ai_model_path.parent.parent))
    try:
        from anomaly_detection.models.autoencoder import SSIMAnomalyAutoencoder
        AUTOENCODER_AVAILABLE = True
    except ImportError as e:
        print(f"[WARN] Could not import autoencoder: {e}")
        AUTOENCODER_AVAILABLE = False
else:
    print(f"[WARN] AI model path not found: {ai_model_path}")
    AUTOENCODER_AVAILABLE = False
...
class AnomalyDetector:
    ...
    def load_model(self):
        ...
        try:
            # 모델 파일 존재 확인
            if not Path(self.model_path).exists():
                print(f"[ERROR] Anomaly model file not found: {self.model_path}")
                self.model = None
                return False

            # Autoencoder 클래스가 import 되었는지 확인
            if not AUTOENCODER_AVAILABLE:
                print("[WARN] Autoencoder class not available, anomaly detection disabled")
                self.model = None
                return False
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                print(f"[ANOMALY] Creating model with config: {config}")
                self.model = SSIMAnomalyAutoencoder(
                ...
                print(f"[ANOMALY] Model loaded successfully (epoch: {checkpoint.get('epoch', 'unknown')})")
                return True
            else:
                # 전체 모델이 저장된 경우
    ...
    def detect(self, frame: np.ndarray) -> Tuple[List[Dict], float, bool]:
        ...
        try:
            ...
            # Tensor 변환 [H, W, C] -> [1, C, H, W]
            frame_tensor = torch.from_numpy(frame_normalized).permute(2, 0, 1).unsqueeze(0)
            frame_tensor = frame_tensor.to(self.device)

            # 이상 점수 계산
            with torch.no_grad():
                anomaly_score, _ = self.model.compute_anomaly_score(frame_tensor, return_map=False)
                reconstruction_error = anomaly_score.item()
...
class ManualInspectionSystem:
    ...
    def __init__(self, config: InspectionConfig, camera: CameraController = None):
        ...
        if self.anomaly.model_loaded:
            print("[SYSTEM] ✅ Anomaly detection model loaded successfully")
        else:
    ...
    def inspect_current_zone(self) -> DetectionResult:
        ...
        elif zone_type == 'anomaly' and self.anomaly.model_loaded:
            # Anomaly zone에서는 Anomaly 모델 사용
            print(f"  🔍 Running Anomaly Detection...")
            detections, inference_time, is_defective = self.anomaly.detect(frame)
            print(f"  Result: {'Anomaly detected' if is_defective else 'Normal'} ({inference_time:.2f}ms)")
            if detections:
                for det in detections:
                    print(f"    - {det['description']}")

        else:
            # Defect zone 또는 Anomaly 모델이 없을 때는 YOLO 사용
            detector_name = "YOLO defect detection"
            if zone_type == 'anomaly':
                print(f"  ⚠️  Anomaly model not available, falling back to YOLO")
