#!/usr/bin/env python3
"""
Lightweight U-Net 검증 스크립트
학습된 U-Net 모델을 테스트 데이터셋으로 검증하고 결과를 시각화/저장

Usage:
    python validate_unet.py --model path/to/best_model.pt
"""
...
# train_unet에서 클래스 import
from training.anomaly.train_unet import (
    LightweightUNet, CombinedLoss, UNET_CONFIG
)
...
VALIDATION_CONFIG = {
    'num_samples': 200,
    'image_size': (256, 256),  # U-Net과 동일
    'batch_size': 16,
    'results_dir': CONFIG_ROOT / 'validation_results' / 'anomaly_unet',
}
...
def load_model(model_path: Path, device: str = 'cuda') -> LightweightUNet:
    """학습된 모델 로드"""
    ...
    base_channels = saved_config.get('base_channels', 32)

    print(f"Loading model with base_channels={base_channels}")

    # 모델 생성 (저장된 config 사용)
    model = LightweightUNet(in_channels=3, base_channels=base_channels).to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Validate Lightweight U-Net model')
    parser.add_argument('--model', type=str,
                        default=str(CONFIG_ROOT / 'models' / 'anomaly_unet' / 'best_model.pt'),
    ...
    print("="*60)
    print("Lightweight U-Net Validation")
    print("="*60)
    ...
    val_image_paths, val_labels = prepare_validation_dataset(
        ...
        normal_category_id=UNET_CONFIG['normal_category_id'],
        anomaly_category_ids=UNET_CONFIG.get('anomaly_category_ids')
    )
    ...
    # 체크포인트에서 config 읽어서 동일한 Loss 사용
    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
    saved_config = checkpoint.get('config', {})

    if saved_config.get('use_mse_only', True):
        criterion = nn.MSELoss(reduction='none')
        print("Using MSE Loss")
    else:
        mse_weight = saved_config.get('mse_weight', 0.5)
        ssim_weight = saved_config.get('ssim_weight', 0.5)
        criterion = CombinedLoss(mse_weight=mse_weight, ssim_weight=ssim_weight)
        print(f"Using Combined Loss (MSE={mse_weight}, SSIM={ssim_weight})")
    ...
    print("="*60)
    print(f"Model: Lightweight U-Net")
